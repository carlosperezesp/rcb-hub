...
WORK_DIR="$(mktemp -d)"
ZIP_PATH="$WORK_DIR/t20s_male_json.zip"
JSON_DIR="$ROOT_DIR/data/cricsheet/t20s_male_json"
...
echo "Downloading Cricsheet T20I JSON zip..."
curl --fail --location --retry 3 --show-error "https://cricsheet.org/downloads/t20s_male_json.zip" -o "$ZIP_PATH"
ls -lh "$ZIP_PATH"
unzip -tq "$ZIP_PATH" >/dev/null

echo "Extracting Cricsheet T20I files..."
rm -rf "$JSON_DIR"
mkdir -p "$JSON_DIR"
unzip -q "$ZIP_PATH" -d "$JSON_DIR"

JSON_COUNT="$(find "$JSON_DIR" -type f -name '*.json' | wc -l | tr -d ' ')"
echo "Found $JSON_COUNT Cricsheet T20I JSON files"
if [ "$JSON_COUNT" = "0" ]; then
  echo "No Cricsheet T20I JSON files found after extraction" >&2
  exit 1
fi
